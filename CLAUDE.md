...
- `.claude/settings.json` wires one `PostToolUse` hook: after any `Edit`, `Write`, or `MultiEdit`, `.claude/hooks/validate-definitions.sh` re-runs the definition validator if the edited path was an agent, a skill, `CLAUDE.md`, or the validator itself, and shows the failures to Claude on exit 2. `PostToolUse` cannot block, so this catches a broken definition immediately after it is written rather than whenever someone remembers to run the script. Test it standalone with `echo '{"tool_input":{"file_path":"<abs path>"}}' | .claude/hooks/validate-definitions.sh`
