...
- `agent-team-workspace/` — the agent-team pipeline's documents: `requirements/<topic>/` (signed-off briefs from `/scoping`), `design-docs/<topic>/` (a design-loop run: its frozen `brief.md`, the `design.md` deliverable, and the `review.md` round ledger), `pull-requests/<topic>/` (pr-loop state), `agent-specs/` (the `brief-spec`, `rfc-spec`, `pr-spec` output contracts), `protocols/` (the two loop protocols), `validate-definitions.py` (runs 100 structural and semantic checks over every agent and skill definition; exits non-zero on failure), and `research/` (standalone investigations and the older ad-hoc agent-team prompts under `legacy-prompts/`). Each document artifact gets its own folder
...
- `.claude/agents/requirements-investigator.md` — subagent that derives requirements from artifacts (running code, tests, schemas, upstream contracts, tickets) rather than from an interview, and writes a `Status: draft` brief to `agent-team-workspace/requirements/<slug>/` per [brief-spec](agent-team-workspace/agent-specs/brief-spec.md). Dispatch it before `/scoping` when the requirements exist but nobody has written them down; it never signs off, and every line it writes is labelled observed / inferred / assumed
- The rest of `.claude/agents/`, by role: `research-investigator` and `design-bar-raiser` (design-loop pair), `coding-expert` and `code-bar-raiser` (pr-loop pair), `ai-writing-auditor` (final editorial pass on both loops' prose), `architect-reviewer` (one-shot review of a design someone else wrote), `code-reviewer` (one-shot diff review outside a loop), `golang-pro` and `rust-pro` (write and modify Go and Rust; `rust-pro` is the only Rust agent), `kubernetes-specialist` (read-only against live clusters, guarded by `.claude/agents/hooks/kubectl-guard.sh`), and `multi-agent-coordinator` (plan how several agents sequence work; produces a plan, does not run it)
- `.claude/skills/` — `review-code` (Go/Python convention pass over working-tree code, with a naming-only mode that absorbed the former review-naming skill), `summarize-changes`, `scoping` (interactive problem-scoping with the user — runs in the main session, grounds requirements in the codebase, produces a signed-off brief per [agent-team-workspace/agent-specs/brief-spec.md](agent-team-workspace/agent-specs/brief-spec.md) that feeds the loops), `design-loop` (entry point for the design-review loop: `research-investigator` authors, `design-bar-raiser` challenges, protocol in [agent-team-workspace/protocols/design-review-loop-agent-team-prompt.md](agent-team-workspace/protocols/design-review-loop-agent-team-prompt.md)), `pr-loop` (entry point for the PR loop: `coding-expert` implements, `code-bar-raiser` reviews, protocol in [agent-team-workspace/protocols/pr-loop-agent-team-prompt.md](agent-team-workspace/protocols/pr-loop-agent-team-prompt.md), contract in [agent-team-workspace/agent-specs/pr-spec.md](agent-team-workspace/agent-specs/pr-spec.md)), `pr-review` (review a PR's code changes, or work through the review comments on one; carries Go, Rust, and C++ depth in `references/` files loaded only when the diff contains that language), `rust-expert` (write Rust interactively — ownership and type design first, then compiler-driven implementation; `references/` carry a borrow-checker error-to-design-fix triage, a Go-to-Rust idiom mapping, and async shape selection), and `agent-factory` (process for creating new subagents and agent-team loops) project-specific skills
